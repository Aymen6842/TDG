...
Chapter~1 established \emph{what} the platform is and \emph{why} it exists. This chapter explains
...
design, the technology choices, and the anticipated risks and challenges. The five development
sprints are narrated in Chapter~3, and the AI sprint in Chapter~4.
...
follows from the shape of the work rather than fashion: the platform spans ten operational
domains (as described in Section~1.3), and the requirements for the later ones --- notifications,
monitoring, and above all the AI copilot --- were only going to sharpen as the earlier ones
landed and were used. A single up-front waterfall specification would have frozen decisions we
were not yet qualified to make. Scrum let us deliver breadth in shippable slices: a working
authentication core first, then projects on top of it, then the agile backlog on top of
projects, so that each increment was demonstrable and the next was planned against something
real.
...
\begin{itemize}
  \item \textbf{Sprint planning}: pull a slice of the product backlog into the sprint backlog and
  ...
  \item \textbf{Backlog refinement}: ran continuously as later domains came into focus.
\end{itemize}
...
The sprint decomposition is reverse-engineered from the delivered system's module dependency
order: the layers had to be built in this sequence (authentication before projects, projects
before the agile backlog, content before the copilot that indexes it), so the plan reflects a
real ordering rather than an invented tracker history.

One additional note: inside Sprint~6, where the RAG copilot is built, we layer a second
methodology --- \textbf{CRISP-DM}~\cite{shearer2000crisp} (Cross-Industry Standard Process for
Data Mining) --- as a sub-process. An ML component needs an evidence loop that ordinary CRUD
features do not: data understanding, modeling, and evaluation. This is detailed in Chapter~4.

\section{Product Backlog}

The backlog re-expresses each delivered module's endpoints and features as user stories in the
...
The plan sequences the backlog into six sprints along a strict dependency order: nothing can be
built before the authentication and RBAC foundation, projects are the container everything agile
hangs off, and the AI copilot is deliberately last because it indexes and answers over content
the earlier sprints produce.
...
\toprule
Sprint & Ch. & Goal & Modules & SP & Duration \\
\midrule
\endfirsthead
...
\bottomrule
\endfoot
S1 --- Foundations \& Auth & 3 & Stand up the layered backend, the DB foundation, and the authentication + RBAC core. & auth/JWT/RBAC, users, teams & 41 & 3w \\
...
S6 --- AI Copilot \& Estimation & 4 & The differentiator: a permission-scoped RAG copilot with grounded answers and task-effort estimation. & AI copilot + estimation & 42 & 4w \\
\end{longtable}

Figure~\ref{fig:2-1} collapses the platform's approximately 31 role types (as described in
Section~1.3) into the six actor archetypes that recur across the sprints and maps them onto the
headline capabilities. Figure~\ref{fig:2-2} is the release roadmap: the six sprints as a
sequence of timeboxes carrying their planned story-point load.

\reportfigure{figures/fig_2_1}{Global use-case diagram: actor archetypes vs.\ platform capabilities.}{2-1}

\reportfigure{figures/fig_2_2}{Project roadmap: six sprints with planned story-point loads.}{2-2}
...
This clean separation means the API could serve any client (mobile, CLI, a second web app)
unchanged. The verified system totals framing the architecture --- 146~route handlers across
20~controllers, 55~Prisma models and 25~enums across 13~schema files --- were produced by
grepping the source. Figure~\ref{fig:2-3} presents the system component view.

\reportfigure{figures/fig_2_3}{System component and deployment view.}{2-3}
...
carries request and response shapes with validation decorators. This strict convention is what
lets any of the approximately 20 backend modules be read the same way. Figure~\ref{fig:2-4}
shows the layering.

\reportfigure{figures/fig_2_4}{Backend layered architecture: controller \textrightarrow{} service \textrightarrow{} repository \textrightarrow{} Prisma.}{2-4}
...
Server state lives in TanStack Query~\cite{tanstackquery}; ephemeral UI state lives in
Zustand~\cite{zustand} stores; forms are validated with Zod~\cite{zod} schema factories that
...
\texttt{docker-compose}~\cite{docker} brings up three backing services:
...
Each app ships a \texttt{Dockerfile} that serves as a reproducible build recipe.
...
\texttt{User} is the hub: it connects to roles, teams, projects (as member or creator), tasks
(as assignee or reporter), work days, events, reminders, notifications, and copilot queries.
\texttt{Project} is the aggregate root of the operational domain: epics, sprints, milestones,
tasks, and document embeddings all foreign-key back to it. The AI module's
...
Figure~\ref{fig:2-8} presents a simplified global entity-relationship diagram showing the main
entities grouped by domain and their key relationships.

\reportfigure{figures/fig_2_8}{Global entity-relationship diagram.}{2-8}
...
\reportfigure{figures/logo_nestjs}{NestJS logo.}{logo-nestjs}

\textbf{NestJS}~\cite{nestjs} (version~11) is the backend framework. It was chosen for its
opinionated, modular architecture that enforces structure at the framework level --- exactly what
a ten-domain monolith needs to stay maintainable. Its decorator-based approach to routing,
dependency injection, guards, and Swagger documentation maps naturally onto the four-layer
convention adopted across the project.

\reportfigure{figures/logo_prisma}{Prisma logo.}{logo-prisma}
...
\reportfigure{figures/logo_gemini}{Google Gemini logo.}{logo-gemini}

\textbf{Google Gemini}~\cite{gemini} provides both the embedding model and the generation model
for the AI copilot. Using a managed API avoided standing up and operating an inference stack
during a time-constrained internship. Gemini's \texttt{flash-lite} variant serves as a cheap,
fast reranking model on its own quota bucket.

\subsection{Frontend}

\reportfigure{figures/logo_nextjs}{Next.js logo.}{logo-nextjs}
...
\reportfigure{figures/logo_react}{React logo.}{logo-react}
...
\reportfigure{figures/logo_tailwind}{Tailwind CSS logo.}{logo-tailwind}
...
\reportfigure{figures/logo_postgresql}{PostgreSQL logo.}{logo-postgresql}
...
store. This is a deliberate fewer-moving-parts decision: keeping embeddings in the same database
as the domain data means retrieval can be permission-scoped in plain SQL, with no separate
vector database to operate, synchronize, or secure.

\reportfigure{figures/logo_redis}{Redis logo.}{logo-redis}

\textbf{Redis}~\cite{redis} is provisioned in the compose stack but not yet wired to its
intended cache and lock path --- a hook reserved for when caching earns the operational cost of a
second datastore. Currently, distributed locking is handled through a Postgres-based
\texttt{SELECT ... FOR UPDATE SKIP LOCKED} pattern.

\reportfigure{figures/logo_docker}{Docker logo.}{logo-docker}
...
\texttt{/api}. Form validation is built on \textbf{Zod}~\cite{zod} schema factories that export
...
Several risks and challenges were anticipated at the design stage and shaped the technical
decisions described above.

\begin{itemize}
  \item \textbf{AI hallucinations.} A language model asked about the company's projects without
    grounding will confabulate plausible answers. This is the central risk of the AI feature
  ...
  \item \textbf{Breadth versus consistency.} Covering ten operational domains in one codebase
    risks a sprawling, inconsistent system. The mitigation is the strict four-layer, per-feature
    architecture enforced across every module, so that any domain reads the same way once the
    pattern is learned.
  \item \textbf{No self-hosted ML infrastructure.} Standing up and operating a GPU-backed
  ...
  \item \textbf{Security across a rich role model.} With approximately 31 role types sharing one
    ...
    route, with finer ownership checks pushed into services where the data lives.
...
This chapter laid the groundwork for the implementation chapters that follow. The project was
...
inside the same datastore as the domain. Chapters~3 and~4 now tell the build story sprint by
sprint.
