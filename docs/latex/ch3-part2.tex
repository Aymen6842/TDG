...
endpoints, all guarded by a single PostgreSQL-backed distributed lock. Where Sprint~3's
...
case is drawn explicitly because it is the seam that binds the sprint: three of the four
...
\reportfigure{figures/fig_3_26}{Check-in sequence diagram.}{3-26}
...
\reportfigure{figures/fig_3_27}{Time \& Attendance class diagram.}{3-27}
...
where each cron acquires a PostgreSQL distributed lock (\texttt{SELECT \ldots{} FOR UPDATE SKIP
LOCKED} with a 55-second TTL), scans for pending items, fans out to the user's enabled
channels, and marks the item processed. This backbone is exactly what Sprint~5 will formalize
...
Figure~\ref{fig:3-32} is the system after four sprints. It is now large enough that legibility,
not completeness of attributes, is the priority: the three earlier packages --- identity
(Sprint~1), the project family (Sprint~2), and the agile/task engine (Sprint~3) --- are shown
in plain style and compacted to their identifying fields, while this sprint's four new
aggregates are grouped into one \textbf{Productivity Suite} package with fuller attributes. The
seams are what make the picture cohere: every new aggregate hangs off \texttt{User}; a
\texttt{WorkSession} owns the \texttt{TaskTimeEntry} rows from the Tasks package (attendance
