...
Sprint~4 proved the multi-channel delivery pattern inside each module; Sprint~5 makes that
backbone a first-class subject in its own right and adds the one consumer that exists only to
feed it. Two modules land together because they are the two halves of one concern:
\emph{reaching people}. The \textbf{notifications} module owns the delivery substrate: the
...
on them, health-checks them every minute, and turns any outage or looming subscription expiry
into an alert routed to the responsible managers over those same four channels. Pairing them
in one sprint is deliberate: monitoring is the clearest end-to-end demonstration of the
...
Figure~\ref{fig:3-33}. Four actor archetypes appear: a \textbf{User} (any authenticated
...
\textbf{Scheduler} actor stands for the automated detectors --- no human triggers a health
check. The scheduler's alert path \texttt{<<include>>}s the same multi-channel notification
use case the human-facing broadcast uses, which is the point of pairing the two modules.
...
\textbf{Functional description.} This module is the delivery substrate the whole platform sits
...
path that crons and services use for the reminders and nudges of Sprints~3 and~4. The
component view of the fan-out is Figure~\ref{fig:3-34}; a single system notification crossing
the channels is Figure~\ref{fig:3-35}.
...
services fan system notifications out through the same substrate.

\textbf{Business rules.}
\begin{itemize}
  \item Channel defaults encode the intended UX: email and push are \textbf{opt-out}
...
\textbf{Class diagram.} Figure~\ref{fig:3-36} shows the module's seven tables. The shape worth
reading is the three-way split between the notification envelope (\texttt{Notification}), its
content (\texttt{NotificationContent}), and the per-recipient inbox row
(\texttt{UserNotification}). The three settings/integration tables hang one-to-one off
...
posture and a \texttt{domain}), records who is responsible via a
...
service), two expiry watchers that walk each row's \texttt{expiredAt} through escalating
notification bands before a paid subscription lapses, and two senders that deliver the alerts.
...
\texttt{UserNotificationSettings} from Module~A), and flips \texttt{isSent:\,true}. The
health-check cycle is Figure~\ref{fig:3-37}; the outbox-to-channels fan-out is
Figure~\ref{fig:3-38}.
...
\begin{itemize}
  ...
  \item The \texttt{isSent:\,false} flag doubles as \textbf{natural dedup}: the detector query
  ...
  \item \texttt{status} (\texttt{Running}/\texttt{Stopped}/\texttt{Maintenance}) is an
    operator-set lifecycle label; health checks alert \textbf{out-of-band} over the channels
    rather than writing the row, and only \texttt{Running} resources are probed.
...
\textbf{Alert fan-out.} Figure~\ref{fig:3-38} shows the two halves of the outbox meeting: the
detector and expiry crons insert unsent notification rows, and the sender crons poll them,
...
The \textbf{notifications surface} comprises the six guarded routes on
...
The \textbf{infrastructure surface} comprises ten guarded routes on a single
...
On the \textbf{client}, notifications are woven through the whole shell rather than sitting on
a page: a bell dropdown in the header shows the unseen count and marks everything seen on
open, a full list page renders the inbox, and a dedicated settings page toggles the channels
with the Telegram \texttt{chatId} input and the ntfy setup steps. Push registration is
...
through --- is exercised by a dedicated unit suite (\texttt{mail.service.spec.ts}). The
...
substrate --- an in-app inbox, FCM push, per-user channel settings, and Telegram/ntfy links ---
that every other module reaches through one shared method for in-app/push and the dedicated
channel services for the rest. Infrastructure monitoring is the payoff: a self-observing loop
...
server-side so the integration is wired end-to-end, normalizing the service \texttt{domain} to
...
hosts that do not always answer politely. Designing the outbox took iteration --- the first
instinct was to let the detector send directly, but separating detection from delivery proved
essential so a slow channel could never stall the per-minute probe loop. The escalating expiry
...
\textbf{Lessons learned.} The outbox pattern was the sprint's clearest win: with detection and
delivery decoupled by a persisted \texttt{isSent} flag, the system gained free deduplication
(an undelivered alert suppresses re-detection) and multi-instance safety fell out of the
...
easy to read, and confirmed by contrast the value of the dispatcher consolidation already
planned. Reusing Sprint~4's lock-then-scan cron shape for all six monitoring crons meant no
new concurrency thinking was needed.
...
Monitoring. The seam that makes the picture cohere is the channel configuration:
...
documented substrate --- inbox, push, and per-user channel configuration --- and proved it
end-to-end with a self-monitoring loop that carries an infrastructure alert from a failed ping
to a manager's chosen channels. With the platform's operational surface complete, the stage is
set for the technical climax: the AI copilot of Chapter~4, which builds on the same
permission-scoped foundations.
