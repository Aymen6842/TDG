% ch3.tex — Chapter 3: The Delivery Platform (V1) — Sprints 1–3
% Compile with XeLaTeX / LuaLaTeX + fontspec (unicode arrows, checkmarks, accents kept as-is).

\chapter{The Delivery Platform (V1): Sprints 1 to 3}

The six sprints are presented over three chapters, one per delivered increment. This chapter
covers the first of them --- \textbf{V1, the delivery platform} --- built by Sprints~1 to~3.

The grouping is not arbitrary: these three sprints are what it takes to reach a system a team can
actually work in. Sprint~1 establishes who a user is and what they may do; Sprint~2 adds the
project, the container every later feature attaches to; and Sprint~3 adds the work itself --- the
agile backlog and the Kanban engine. Only at the end of Sprint~3 can someone log in, open a
project, and move a task to done. Everything before that point is foundation, and everything
after it (Chapters~4 and~5) extends a platform that already functions. The order is fixed by the
dependency chain established in the sprint plan of Section~2.3: nothing can be built before
authentication, and nothing project-scoped before the project exists.

Every sprint in this chapter and the next follows the same structure. It opens with its goal and
backlog, analyses the requirements per module, and presents the UML design next to the features
it describes. It then reports the implementation with screenshots, validates the deliverables
through testing, and closes with a review, a retrospective, and a cumulative class diagram
showing how the domain model grew. Chapter~4 continues with the operational layer of Sprints~4
and~5, and Chapter~5 covers the AI sprint, the project's main technical contribution.
...
Sprint~1 builds the foundation the rest of the platform depends on: the layered backend and the
identity core.

\subsection{Sprint goal}

Build the base that the rest of the platform depends on: a layered NestJS backend with a
Prisma multi-file schema, and the identity core --- authentication (login, token refresh,
password reset), role-based access control, and administration of users and teams. Nothing in
later sprints can be built or even reached until a caller can be identified and authorized,
which is why this work comes first.

\subsection{Sprint backlog}

The user stories for this sprint, repeated from the product backlog in Section~2.2 so that the
sprint can be read on its own:
...
The sprint delivers two modules. The features, actors, and use cases are shown in
Figure~\ref{fig:3-1}.
...
\begin{itemize}
  ...
  \item A single \texttt{HasPermissionGuard}, applied on 138 routes across 18 controllers, checks
  ...
  \item Finer ownership distinctions (\texttt{*.own} vs \texttt{*.any}) are resolved inside each
    service, where the data needed for that decision is available.
...
success creates the token pair and stores the refresh row.
...
The \textbf{authentication surface} consists of eight endpoints: \texttt{POST /auths/register},
...
The \textbf{administration surface} consists of fourteen guarded endpoints across \texttt{/users}
...
On the \textbf{client}, the guest route group contains the login, registration, and
reset-password screens. The authenticated \texttt{users} section contains the directory table
...
The e2e suite (\texttt{test/agile-permissions.e2e-spec.ts}) runs the full permission matrix
...
administration governed by a single declarative RBAC catalogue applied uniformly across 138
...
\textbf{Difficulties encountered.} This sprint required a lot of learning, as it was our
first experience with both NestJS and Prisma. Understanding NestJS's decorator-based
dependency injection, guard composition, and module wiring took time before productive
development could begin. Learning Prisma's multi-file schema, migration workflow, and query API
added to this initial period. Designing the RBAC catalogue --- mapping approximately 120
permissions across 31 role types into a single declarative constant --- was a real design
effort: the permission granularity had to be adjusted several times to find the right balance
...
four-layer controller-service-repository-DTO convention) showed its value immediately: once the
pattern was established in this sprint, every following module could be scaffolded quickly
by copying the same structure. The table-driven RBAC approach also proved useful early ---
...
same diagram, highlighting only their additions, so the reader can follow how the model grows.
...
the base that the entire domain model and every later sprint build on.
...
Sprint~2 adds the project --- the container that every later feature attaches to.

\subsection{Sprint goal}

With identity and access in place, Sprint 2 introduces the unit of work that everything else
in the platform depends on: the \textbf{project}. The goal was the full project lifecycle (create,
read, update, archive, hard-delete), per-project team membership with a single project manager,
and an email/token invitation flow for bringing people who are not yet members onto a project.
This is the largest domain module and the aggregate root of the whole agile domain: epics,
sprints, tasks, milestones, labels and reminders all hold a foreign key back to a
\texttt{Project}, so it had to be delivered before any of the planning or execution work in the
sprints that follow.

\subsection{Sprint backlog}

The following stories, taken from the product backlog in Section~2.2, form the sprint backlog:
...
The sprint delivers one module. Figure~\ref{fig:3-8} shows the features, actors, and use
cases.
...
\texttt{details}) are stored in a child \texttt{ProjectContent} table keyed by language, following
the app-wide i18n content pattern. WIP limits per kanban column are stored as a JSON map on the
...
\begin{itemize}
  ...
  \item A non-executive caller is restricted to projects they belong to
    (\texttt{members.some(\{ userId \})}); a unit executive is restricted to their \texttt{businessUnit};
...
name raises a Prisma \texttt{P2002} error, which is returned as a \texttt{409}.
...
\texttt{User} is shown again --- not new --- because it now has three associations with the
...
The \textbf{project surface} consists of seventeen endpoints on \texttt{ProjectsController} covering
...
and drag-reorder. Opening a project shows a tabbed detail view; its \textbf{Members} tab lists
members and pending invitations with direct-add, invite-by-email, role-toggle, remove, and
...
with full lifecycle management, business-unit scoping enforced inside the query itself rather
than applied after it, a single-manager invariant guarded on every mutation, and an email-based
invitation flow with identity-verified token acceptance. It is also the first module with a dedicated
...
where the permission predicate and the database query had to be a single atomic statement.
Getting the business-unit scoping right --- CEO global, CTO/CMO limited to their unit, manager
limited to their projects --- required building the Prisma \texttt{where} clauses carefully. The
single-manager invariant also needed care: enforcing it across create, demotion, and removal
without leaving an inconsistent window required validating at three separate points in the
service.

\textbf{Lessons learned.} Pushing the access check into the query itself (rather than fetching
first and filtering after) proved to be the strongest pattern in the module: it removes any gap
between the access check and the data fetch. The i18n content-table pattern
(\texttt{ProjectContent} keyed by language) also proved reusable --- it was adopted directly in
later sprints for sprints and tasks.
...
their enums --- are highlighted. The link back to \texttt{User} (creator, member, inviter) is
what ties the new project aggregate to the identity core established previously.
...
Sprint~3 builds the working core of the platform: the agile backlog and the task board.
...
points, time logging, labels, and threaded comments. They are delivered together because a
sprint is only meaningful once tasks can be assigned to it, and the burndown chart reads
directly from each task's \texttt{storyPoints} and \texttt{completedAt}. At \textbf{64 story
points} over five weeks this is the heaviest load in the plan and the most complex domain code
in the codebase.

\subsection{Sprint backlog}

The sprint backlog below repeats the relevant stories from the product backlog in Section~2.2:
...
The sprint delivers two modules. The features, actors, and use cases are summarized in
Figure~\ref{fig:3-13}.

{\renewcommand{\figmaxheight}{0.75\textheight}%
\reportfigure[\textwidth]{figures/fig_3_13}{Sprint 3 use-case diagram.}{3-13}}

Four Scrum roles appear, each mapped to a capability helper the service enforces. A
\textbf{Project Manager} and \textbf{Product Owner} manage the structure --- epics, milestones,
task creation, dependencies, labels, and board columns. A \textbf{Scrum Master} governs the sprint
...
project. An \textbf{epic} is a large feature that groups tasks under a name and a colour, with
optional dates, so progress can be rolled up. A \textbf{sprint} is a time-boxed iteration with
planned and estimated date windows, a story-point \texttt{capacity}, multilingual content, file
attachments, and a real lifecycle. A \textbf{milestone} is a target date for a set of tasks.
Epics and sprints are restricted to \texttt{AGILE} projects by an \texttt{AgileOnlyGuard}, while
milestones have no such guard because a deadline is useful to a \texttt{FREESTYLE} project too.
All three follow the project-wide four-layer pattern (controller → service → repository → DTO),
each with four thin repositories, and all three enqueue an AI embedding job on every write via
the \texttt{IndexOutboxService}, so agile artefacts are first-class documents for the copilot
built in Sprint~6.
...
\begin{itemize}
  ...
  \item Moving to \texttt{Running} or \texttt{Completed} sends a notification to every project
...
\textbf{Sprint analytics sequence.} The planning artefacts become visible results through the
analytics, computed server-side from real task data. Burndown (Figure~\ref{fig:3-15}) is the
clearest example: for a chosen sprint the service loads every task's \texttt{storyPoints},
\texttt{status}, \texttt{completedAt} and \texttt{createdAt}, sums the committed points, then
goes day by day across the sprint window, computing an \emph{ideal} line (total minus an even
daily burn) against the \emph{actual} remaining (total minus the points of tasks marked
\texttt{DONE} on or before that day). Velocity aggregates completed-sprint points across the
project, and the Gantt view combines four parallel queries --- milestones, epics, sprints,
tasks --- into one timeline payload.
...
\textbf{Sprint lifecycle state machine.} The most complex piece of business logic in this module
is the sprint state machine (Figure~\ref{fig:3-16}). A sprint is always created \texttt{Pending}.
...
\texttt{Completed} can be restarted back to \texttt{Running}. Two side effects are attached to the
transitions: moving to \texttt{Running} or \texttt{Completed} sends a notification to every
...
\textbf{Functional description.} The task engine is the core unit of work and the most complex
domain module in the codebase (\texttt{tasks.service.ts} alone is 2\,735 lines). A task carries
the expected fields --- type, priority, assignee, reporter, story points, estimated and actual
hours, due date --- plus everything a real board needs: a free-string \texttt{status}, a
...
project gets a simple three-column board (\texttt{TODO / IN\_PROGRESS / DONE}) and a progress
...
a system enum name or a custom column name, which lets a team add new columns without a schema
...
moves their own cards across columns, logs time, and comments. Authorization is organized into
seven tiered capability helpers --- from \texttt{canAccessProject} (any member may read, comment,
and log time) up through \texttt{canCreateTaskForProject}, \texttt{canManageTaskStructure},
...
\textbf{Create-task sequence.} Figure~\ref{fig:3-18} shows the create path, which validates
three things before it writes: the caller's capability, the project-type field rules (rejecting a
...
creates due-date reminders, fires a fire-and-forget assignment notification if the assignee is
not the task's creator, and enqueues the AI index upsert.
...
\texttt{TESTING} can move back to \texttt{IN\_REVIEW} or advance to \texttt{DONE}, and \texttt{DONE} can
...
report. \texttt{Task} is the central class; \texttt{ProjectTaskStatus} holds the board columns and their
...
labels. On the client, all of this sits inside the \textbf{project-detail} page rather than in
separate routes. The project opens onto Tasks, Kanban, Backlog, Sprints and Milestones tabs.
Clicking a card opens a detail sheet composed of comment, attachment, dependency, label, subtask
and time-entry sections, and the analytics tab renders the burndown and velocity charts and the
Gantt.
...
Eleven property-based test suites (\texttt{fast-check} + \texttt{vitest}) verify client-side
...
validation. Property-based testing generates hundreds of random inputs per run and asserts that
an invariant holds across all of them, which is a stronger guarantee than a small set of example
cases.
...
\textbf{Objectives achieved.} The sprint delivered the core of the platform: a governed
...
is a true workflow engine --- custom columns, per-column WIP limits, dependency-blocking, and
a circular-dependency guard, all applied under a tiered, project-scoped capability model that
even lets an assignee advance their own card. Every write also feeds the AI index outbox, so the
whole agile and task corpus is already searchable before Sprint~6.

\textbf{Remaining work.} Hardening items identified during the sprint --- deeper backend
coverage of the tasks module, transaction-wrapped agile writes, and a shared
...
database schema alone. The tiered capability model --- seven distinct helpers from read-only
access up to workflow advancement, with an assignee-specific override --- required care to avoid
either granting too much or blocking legitimate moves. The sprint lifecycle state
...
critical write path fast without losing user feedback. The AI embedding outbox --- enqueueing
an index upsert on every agile and task write --- was easy to connect and ensures the
entire corpus is searchable by the time the copilot arrives in Sprint~6.

\textbf{Improvements planned.} Extend backend coverage of the tasks module's transition
...
attachment points are clear: every agile artefact is attached to \texttt{Project}, and \texttt{Task}
...
columns, transitions, WIP limits, and dependency gates are enforced end-to-end. The tiered
capability model and the AI embedding outbox introduced here are carried forward into every
later sprint.

\section{Chapter Conclusion}

Three sprints and 131 story points produced V1: a platform a team can actually work in. A user
authenticates against a stateless JWT core and is authorized by one centralized RBAC catalogue
(Sprint~1); executives create projects scoped to a business unit and assemble their teams by
direct membership or tokenized email invitation (Sprint~2); and inside a project, work is planned
as epics, sprints and milestones and executed on a data-driven Kanban whose columns, transitions,
WIP limits and dependency gates are enforced end to end, with burndown, velocity and Gantt
analytics computed server-side from real task data (Sprint~3).

Three patterns established here govern everything that follows. \textbf{Two-tier authorization}
--- a coarse permission guard at the route, then a data-scoped check inside the service --- is
the template every later module reuses. \textbf{Query-scoped access}, pushing the permission
predicate into the Prisma \texttt{where} clause so the check and the fetch are one atomic
statement, removes any gap between deciding and reading; it is the same principle that later
makes the copilot's retrieval safe by construction. And the \textbf{AI embedding outbox}, wired
into every agile and task write from Sprint~3 onward, means the corpus the copilot will answer
from is already accumulating three sprints before that copilot exists.

What V1 does not yet have is the layer around the work: the personal routines each employee uses
daily, and the communication backbone that makes the platform reach people rather than wait to be
opened. That is the subject of Chapter~4.

\fi % \ifpartialbuild (Sprints 2-3 + chapter conclusion guard, opened before Sprint 2's heading)
