...
estimator --- sharing the same indexing infrastructure and retrieval engine. The features, actors,
and use cases are collected in Figure~\ref{fig:4-1}.
...
\begin{itemize}
  \item Retrieval is \textbf{permission-scoped in SQL}: a user can only surface content from
    projects they may access; out-of-scope rows are never returned to rank, making
    cross-project leakage unrepresentable rather than merely checked.
...
\textbf{The data model.} Three tables carry the subsystem, modeled together in a dedicated Prisma schema file; the
...
\texttt{actualHours}. Two baselines calibrate the numbers: the \emph{project mean} (predict every task at its project's
...
similarity-weighted hours-per-point rate) cuts MAE to \textbf{2.12\,h}, matching the story-points baseline (2.10\,h)
while returning what no regression line can --- the neighbour tasks as inspectable evidence and an honest
uncertainty band that covers the true value 74\% of the time (against the \textasciitilde80\% its 10/90 band nominally
targets). In the shipped form the suggestion is size-agnostic (a draft being typed carries no points yet); the
size-aware mode activates whenever a pointed draft is estimated through the API.
...
of the indexing pipeline --- with its outbox seam, retry backoff, and nightly reconcile --- also
made end-to-end verification more involved than synchronous modules.

\textbf{Lessons learned.} Building the offline evaluation harness early proved invaluable: it
caught the keyword-regime retrieval gap before the feature shipped and validated the hybrid fix
quantitatively. The CRISP-DM framing also paid off --- naming the phases made it explicit that
the evaluation harness is the quality gate standing in for a trained model's validation curve,
...
the domain through a deliberately loose outbox seam and attaches to the cumulative data model
via soft references, keeping the AI subsystem's concerns self-contained while reading from the
full breadth of project content accumulated over the five preceding sprints.
