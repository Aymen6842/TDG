\chapter{The Intelligence Layer (V3): Sprint 6 --- AI Copilot \& Estimation (RAG)}
...
\begin{itemize}
  ...
  citation precision/recall, refusal correctness, and estimation error --- the subject of Section~5.8.
\end{itemize}
...
are carried into the sprint review (Section~5.10).
...
This is the one sprint that closes without a Given/When/Then acceptance table, and the reason is
the nature of the deliverable rather than an omission. A CRUD endpoint either returns the right
row or it does not, so a Given/When/Then line settles it. A retrieval pipeline has no single
correct output: the same question can be answered well by several different passages, so
``correct'' is a ranking quality measured over a set of questions, not a condition met on one.
The metrics tables in this section are therefore this sprint's acceptance evidence, and they are
a stricter bar than the tables they replace --- each of the seven stories is judged against a
...
\midrule
Cross-role leakage & out-of-scope hits & \textbf{0} in both arms & confirms the in-SQL scoping of Section~5.6 \\
\bottomrule
\end{longtable}
...
\textbf{Threats to validity.} These numbers are measured, but they are measured on a small
internal corpus, and it is worth stating plainly what that does and does not support. The corpus
is 73 chunks; the gold sets hold 12 semantic and 10 keyword questions, the QA set 10 questions,
and the estimator was evaluated over 43 completed tasks. The faithfulness mean is over n = 6.
Sets this size give wide confidence intervals, so a single metric should be read as a direction,
not as a benchmark score --- and the semantic set is small enough that the bi-encoder saturates
it, which is precisely why the keyword set was built as a second, harder probe. Three further
limits apply. The gold sets were written by the same author who built the retriever, so they
test whether the system meets its intended behaviour rather than an independent notion of
relevance. The faithfulness judge is itself a language model, so answer quality is assessed by
the same class of system being assessed. And the reranker's score varies slightly between runs
because it is an LLM pass, which is why the report reads it as ``never regresses, sometimes
improves'' rather than quoting a single figure. The estimation result carries its own caveat,
stated in the paragraph above: the size-aware predictor matches the story-points baseline rather
than beating it, and its contribution is the neighbour evidence and the calibrated uncertainty
band, not a lower error. What the evaluation does establish firmly is the comparison it was
built for --- hybrid retrieval against a vector-only baseline on the same questions, same corpus,
same day --- and the permission-scoping result, where zero cross-role leakage follows from the
SQL filter rather than from a sample.
...
\begin{itemize}
  ...
  is effective but has a known limitation --- it ignores the lexical arm --- which we carry into the review (Section~5.10).
...
\begin{itemize}
  ...
  refused --- so the copilot's answer path does not fully inherit the hybrid keyword improvement demonstrated in Section~5.8. Making
...
a missing fail-fast on the Gemini key, and unit coverage of the pure functions (RRF fusion,
