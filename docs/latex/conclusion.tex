...
the ten operational domains that Chapters~3 to~5 present sprint by sprint: projects and membership,
...
fourth objective --- the intelligent layer --- is met by the RAG subsystem of Chapter~5, and it is the one the
...
retrieve from disjoint candidate sets, and the evaluation measured \textbf{zero cross-role leakage} (Sections~5.6
and~5.8). Its freshness is maintained by a \textbf{transactional-outbox boundary} processed by a locked sweeper, with a
self-healing nightly reconcile, so the domain never depends on the AI subsystem to save its data (Section~5.5).
...
hybrid lexical+vector design (Section~5.8). In a report built entirely from evidence, this is the one
...
\begin{itemize}
  ...
  \item \textbf{Testing.} The suite pairs unit tests, an end-to-end RBAC permission matrix, property-based
  suites over the client transform layers, and the AI evaluation harness. The natural next step is to
  extend behavioural coverage across the remaining domain modules, targeting the paths the correctness
  items above name --- a business-day-boundary test, for instance, is what would pin the check-in dead zone.
  \item \textbf{DevOps.} The platform is fully containerized --- one \texttt{docker compose up} builds and
  starts both applications and their backing services, provisions the schema through
  \texttt{prisma migrate deploy}, and persists uploads in a named volume --- so the stack is reproducible
  on any machine with Docker. What remains is the step beyond reproducibility: there is no CI/CD
  pipeline, and the client's \texttt{BACKEND\_ADDRESS} is inlined into the browser bundle at build time,
  so retargeting the platform at a non-local host means rebuilding the web image rather than changing a
  runtime variable. The platform has therefore been run and validated locally but is not deployed to a
  public environment --- the security items above are the prerequisite for that step.
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{DevOps.} Building on the existing compose stack: add CI (lint + build +
   \texttt{prisma validate} + the existing suites), make \texttt{BACKEND\_ADDRESS} runtime-configurable
   so the web image no longer has to be rebuilt per environment, and move uploads to object storage
   once the platform runs on more than one host.
