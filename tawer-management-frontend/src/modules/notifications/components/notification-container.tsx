...
export default function NotificationContainer({ notification }: Props) {
  ...
  return notification.url ? (
    <Link
      ...
      <div className="flex flex-1 flex-col gap-1.5">
        <div className="flex items-center justify-between gap-2">
          <div
            className={`truncate text-sm font-semibold ${!notification.isSeen ? "text-foreground" : "text-muted-foreground"}`}>
            {notification.title}
          </div>
          {!notification.isSeen && <span className="bg-destructive h-2 w-2 rounded-full" />}
        </div>
    ...
    <div className="flex flex-1 flex-col gap-1.5">
      <div className="flex items-center justify-between gap-2">
        <div
          className={`text-sm font-semibold max-w-full  ${!notification.isSeen ? "text-foreground" : "text-muted-foreground"}`}>
          {notification.title}
        </div>
        {!notification.isSeen && <span className="bg-destructive h-2 w-2 rounded-full" />}
      </div>
