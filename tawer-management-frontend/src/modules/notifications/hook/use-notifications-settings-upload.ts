...
export default function useNotificationsSettingsUpload() {
  ...
  useEffect(() => {
    if (settings) {
      if (settings.emailNotificationsEnabled !== emailNotifications)
        setEmailNotifications(settings.emailNotificationsEnabled);
      if (settings.telegramNotificationsEnabled !== telegramNotifications)
        setTelegramNotifications(settings.telegramNotificationsEnabled);
      if (settings.ntfyNotificationsEnabled !== ntfyNotifications) setNtfyNotifications(settings.ntfyNotificationsEnabled);
    }
  }, [settings])
