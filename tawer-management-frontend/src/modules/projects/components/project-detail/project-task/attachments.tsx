...
import { getAttachmentStoragePath } from "@/modules/projects/utils/resolve-attachment-url";
...
export default function AttachmentsUpload({ inputName, previews, resetTrigger }: Props) {
  ...
  // Sync files to form field outside of render cycle
  useEffect(() => {
  ...
  const removeAlreadyCreatedAttachment = (attachment: string) => {
    if (previews) {
      ...
      const attachmentPathname = getAttachmentStoragePath(attachment);
                ...
                <div className="space-y-2">
                  {alreadyCreatedAttachments.map((attachment, idx) => (
                    <AttachementPreview
                      ...
                      onDeleteAttachment={() => removeAlreadyCreatedAttachment(attachment)}
