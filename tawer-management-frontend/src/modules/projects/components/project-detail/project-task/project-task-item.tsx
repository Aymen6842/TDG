...
export default function ProjectTaskItem({
  ...
  const epicName = epics?.find(e => e.id === task.epicId)?.name || task.epicId;
  const assigneeName = project?.members?.find(m => m.userId === task.assigneeId)?.user?.name || task.assigneeId;

  console.log("DEBUG Task ID:", task.id);
  console.log("DEBUG Epic ID:", task.epicId, "Found in epics:", epics?.find(e => e.id === task.epicId));
  console.log("DEBUG epics array length:", epics?.length);
          ...
          <CardFooter className="flex flex-wrap gap-2 border-t py-3">
            ...
            {visibleAttributes.milestone && task.milestoneId && (
              <div className="flex w-full items-center gap-1 text-muted-foreground">
                ...
                <span className="text-[10px] truncate">{milestoneName}</span>
              </div>
            )}
          </CardFooter>
        </Card>
