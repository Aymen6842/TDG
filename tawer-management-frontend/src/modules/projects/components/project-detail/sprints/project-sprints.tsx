...
export default function ProjectSprints({ project }: Props) {
  ...
  const handleStatusChange = async (sprint: SprintType, status: SprintStatus) => {
    try {
      await uploadSprint(project.id, { status }, sprint.id);
      queryClient.invalidateQueries({ queryKey: ["project-sprints", project.id] });
      toast.success(t(`statusToast.${status.toLowerCase()}`, { defaultValue: `Sprint ${status.toLowerCase()}` }));
    } catch (err: any) {
      const msg = err?.response?.data?.message || t("statusToast.error", { defaultValue: "Failed to update sprint status" });
      toast.error(msg);
    }
  };
      ...
      <div className="space-y-4">
        ...
        {sprintsAreLoading ? (
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {sprints.map((sprint) => (
              <SprintCard key={sprint.id} sprint={sprint} viewMode="grid" onEdit={(s) => { setEditSprint(s); setIsSheetOpen(true); }} onDelete={setSprintToDelete} onStatusChange={handleStatusChange} />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 space-y-4">
            {sprints.map((sprint) => (
              <SprintCard key={sprint.id} sprint={sprint} viewMode="list" onEdit={(s) => { setEditSprint(s); setIsSheetOpen(true); }} onDelete={setSprintToDelete} onStatusChange={handleStatusChange} />
            ))}
          </div>
        )}
      </div>

      <SprintUploadSheet projectId={project.id} projectStartDate={project.startTime} projectEndDate={project.endTime} isOpen={isSheetOpen} onClose={() => { setIsSheetOpen(false); setEditSprint(null); }} sprint={editSprint} />
