...
export default function useProjects() {
  ...
  // A client filter is only "active" if it deviates from its default/neutral state
  const hasClientFilter = !!(projectType || isArchived === true || createdById);
  ...
  const lastProcessedData = React.useRef<{ data: ProjectType[]; pagination: any } | null | undefined>(undefined);

  // Reset pool whenever server-side params change
  ...
  if (prevServerKey.current !== serverKey) {
    ...
    setPool([]);
    lastProcessedData.current = undefined;
    setBackendPage(1);
  ...
  // Client-side filter changes only reset the display page — pool stays intact
  ...
  // Derive creators from the pool — only users who actually created a loaded project
  const creators = React.useMemo(() => {
    const seen = new Map<string, { id: string; name: string; isMe: boolean }>();
    for (const p of pool) {
      if (p.createdBy && !seen.has(p.createdBy.id)) {
        seen.set(p.createdBy.id, {
          id: p.createdBy.id,
          name: p.createdBy.name,
          isMe: p.createdBy.id === user?.id,
        });
      }
    }
