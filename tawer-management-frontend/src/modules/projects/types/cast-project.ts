...
export function castProjectToFrontend(raw: ProjectInResponseType): ProjectType {
  ...
  const members: ProjectMember[] = (raw.members ?? []).map((m) => ({
    ...
    user: m.user ?? (m.memberName ? { id: m.userId, name: m.memberName } : undefined),
  }));
  ...
  const name = raw.name ?? content?.name ?? "Unnamed Project";

  return {
    id: raw.id,
    slug: name.toLowerCase().replace(/ /g, "-") || raw.id,
    name,
