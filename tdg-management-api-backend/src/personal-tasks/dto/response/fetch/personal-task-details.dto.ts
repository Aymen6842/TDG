...
import { PersonalTaskCommentDto } from './personal-task-comment.dto';
import { PersonalTaskSummaryDto } from './personal-task-summary.dto';

export class PersonalTaskDetailsDto {
  @ApiProperty({
  ...
  @Expose()
  @Transform(
    ({ obj }) => (obj as PersonalTaskDetailsDto)?.content?.[0]?.title ?? null,
    {
      toClassOnly: true,
    },
  )
  ...
  @Transform(
    ({ obj }) =>
      (obj as PersonalTaskDetailsDto)?.content?.[0]?.description ?? null,
    {
      toClassOnly: true,
    },
  )
  ...
  @Expose()
  @Transform(
    ({ obj }) => (obj as PersonalTaskDetailsDto)?.content?.[0]?.details ?? null,
    {
      toClassOnly: true,
    },
  )
  ...
  @ApiProperty({
    ...
    type: PersonalTaskCommentDto,
    isArray: true,
  })
  @Type(() => PersonalTaskCommentDto)
  comments: PersonalTaskCommentDto[];
  ...
  @Type(() => PersonalTaskSummaryDto)
  subTasks: PersonalTaskSummaryDto[];
