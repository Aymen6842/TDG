...
import { ServerServiceStatus, UserServerManagement } from '@prisma/client';
import { Exclude, Transform } from 'class-transformer';
import { TimeService } from 'src/common/time/service/time.service';

export class UpdatedServerDto {
  ...
  managers?: UserServerManagement[];

  @ApiProperty({
    description: 'List of manager IDs associated with the server',
    example: ['managerId1', 'managerId2'],
  })
  @Transform(
    ({ obj }) => (obj as UpdatedServerDto).managers?.map((m) => m.managerId),
    { toClassOnly: true },
  )
  managersIds?: string[];
